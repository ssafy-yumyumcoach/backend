...
public class UserService {
    ...
    public MyPageResponse getMyPage(String email) {
        ...
        // ✅ current가 없을 수 있으니 null-safe 처리
        MyTitleResponse current = userTitleMapper.findCurrentTitle(email);

        // ✅ titles 리스트도 null로 올 수 있으면 안전하게 빈 리스트로
        List<MyPageResponse.TitleItem> myTitles =
                Optional.ofNullable(userTitleMapper.findMyTitles(email))
                        .orElse(List.of());

        Long currentTitleId = (current != null) ? current.getCurrentTitleId() : null;
        String currentTitleName = (current != null) ? current.getCurrentTitleName() : null;
